...
public class ThemeController {
	...
	public ResponseEntity<List<ThemeModel>> getByNome(@PathVariable String nome) {
		return ResponseEntity.ok(repository.findAllByDescricaoContainingIgnoreCase(nome));
	}

	@PostMapping
	public ResponseEntity<ThemeModel> post(@RequestBody ThemeModel tema) {
		return ResponseEntity.status(HttpStatus.CREATED).body(repository.save(tema));
	}

	@PutMapping
	public ResponseEntity<ThemeModel> put(@RequestBody ThemeModel tema) {
